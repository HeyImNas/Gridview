...
async def fetch_twitch_streams(token, valid_streamers, title_blacklist, groups_data, channel_allowlist):
...
async def fetch_kick_streams_from_lofi(session, groups_data, channel_blacklist):
    """Extracts raw Kick stream data from Lofi-Nopixel and enforces strict GTA V category."""
    try:
        ...
        for block in blocks[1:]:
            ...
            # STRICT CATEGORY FILTER (Kick Game ID for GTA V is "8")
        ...
        logger.info(f"Successfully extracted {len(kick_streams)} Kick streams.")
        return kick_streams
...
async def fetch_streams_loop():
    ...
    # Initialize in the past so they trigger immediately on boot
    ...
    async with AsyncSession(impersonate="chrome116") as session:
        while True:
            ...
            # --- KICK Refresh ---
            kick_data = await fetch_kick_streams_from_lofi(session, groups_data, channel_blacklist)
            
            if kick_data is not None:
                current_cycle_kick = []
                for s in kick_data:
                    # Enforce title blacklist (e.g., blocking Prodigy RP)
            ...
            # --- TWITCH Refresh (Every 5 Minutes) ---
            if (now - last_twitch_update).total_seconds() >= 300:
                try:
                    ...
                    if token and valid_twitch_streamers:
                        last_twitch_results = await fetch_twitch_streams(
            ...
            raw_merged = last_kick_results + last_twitch_results
            deduped_streams = []
